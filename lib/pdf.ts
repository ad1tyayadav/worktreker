...
const drawThinLine = (pdf: jsPDF, x1: number, y: number, x2: number, color: [number, number, number] = COLORS.borderLight) => {
  pdf.setDrawColor(...color);
...
export const generateInvoicePdf = (options: PdfOptions, filename?: string) => {
  const name = options.invoice.title || options.invoice.invoice_number || "invoice";
  generateTextPdf(options, filename || `${name}.pdf`);
};
...
function generateTextPdf(options: PdfOptions, filename: string) {
  ...
  if (isRetro) {
    ...
    drawThinLine(pdf, MARGIN, y, PAGE_W - MARGIN);
  }
  y += 6;
  ...
  if (selectedEntries.length === 0) {
    ...
    for (const entry of selectedEntries) {
      ...
      if (isRetro) {
        ...
        drawThinLine(pdf, MARGIN, y - 1, PAGE_W - MARGIN);
      }
    }
  }
  ...
  pdf.setFontSize(7.5);
  pdf.setTextColor(...COLORS.muted);
  pdf.text("Generated by worktreker.online", PAGE_W / 2, y, { align: "center" });
